...
- [ ] **21. Extract events + shop to config-driven JSON** — (a) Move event definitions from hardcoded JS arrays in `events.js` to `data/events.json`. Schema: name, duration, modifiers (with min/max validated ranges), reward tiers, scheduled start/end dates. (b) Move shop offer definitions from `shop.js` to `data/shop.json`. Schema: name, price, contents, display order, availability window. Both load at init with graceful fallback if JSON is malformed.
